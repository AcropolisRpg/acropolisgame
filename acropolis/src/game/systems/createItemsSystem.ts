    ...
    if (window.acropolis.currentPlayerId && !executed) {
      ...
      if (entity?.sprites) {
        ...
        if (networkEntity.items) {
          ...
          console.log('inventoryLayout', inventoryLayout);
          if (entity.items) {
            // Layout 16x64
            for (const [key, networkItem] of Object.entries<any>(
              ...
              if (!entity.sprites.items[key]) {
                ...
                console.log('inventoryLayout', inventoryLayout);
                // .find(inventoryLayout, { 'age': 1, 'active': true });
                ...
                // const group = scene.add.group(entity.sprites.items[key].sprite, entity.sprites.items[key].qty)
                scene.input.setDraggable(entity.sprites.items[key].sprite);

                // scene.input.setDraggable(entity.sprites.items[key].qty);
                x += 32;
                counter++;
                if (counter > 8) {
                  y += 32;
          ...
          (
            ...
            if (objectsClicked?.[0]?.name === 'backpack') {
              ...
              if (entity?.sprites?.items) {
                for (const [, item] of Object.entries<any>(
                  entity.sprites.items
                )) {
