...
export const createBroadcastNetworkSystem = () => {
  return defineSystem((world) => {
    ...
    for (const [networkEntityId, networkEntity] of Object.entries(global.networkEntities)) {
      const { position, type, target, id, category, action, items, skillPosition, healthPoints } = networkEntity
      if (type === 'player') {
        global.broadcastNetworkClient[networkEntityId] = { position, type, target, id, action, items, skillPosition, healthPoints }
      }
      if (type === 'resource') {
